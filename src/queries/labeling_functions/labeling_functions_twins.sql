drop table if exists general.same_year_continue_twins_pairs;

create table
general.same_year_continue_twins_pairs
as
select
...
# performance metrics
, avg(if(cont.owned_repository >= no_cont.owned_repository, 1,0)) as owned_repository_avg_better
, avg(if(cont.owned_repository = no_cont.owned_repository, 1,0)) as owned_repository_avg_same
, avg(if(cont.commits >= no_cont.commits, 1,0)) as commits_avg_better
, avg(if(cont.files_edited >= no_cont.files_edited, 1,0)) as files_edited_avg_better
, avg(if(cont.ccp <= no_cont.ccp, 1,0)) as ccp_avg_better
, avg(if(cont.same_date_duration_avg <= no_cont.same_date_duration_avg, 1,0)) as same_date_duration_avg_avg_better
, avg(if(cont.commit_period >= no_cont.commit_period, 1,0)) as commit_period_avg_better
, avg(if(cont.commit_days >= no_cont.commit_days, 1,0)) as commit_days_avg_better
FROM
general.developer_motivation_profile as cont
join
general.developer_motivation_profile as no_cont
...
# into continue_twins_by_age.csv
SELECT
case
...
# into continue_twins_by_language.csv
SELECT
cont_r.language_code as f
, count(distinct cont.author_email) as developers
, count(*) as cnt
, avg(if(cont.message_length_avg>= no_cont.message_length_avg, 1,0)) as message_length_avg_better
, avg(if(cont.refactor_mle>= no_cont.refactor_mle, 1,0)) as refactor_mle_avg_better
...
not if(no_cont.status='Continue'
          ...
          , false)
and
cont_r.language_code = no_cont_r.language_code
group by f
having count(distinct cont.author_email) >= 100
order by f
;
...
# into continue_twins_by_is_company.csv
SELECT
cont_r.Is_Company as f
, count(distinct cont.author_email) as developers
, count(*) as cnt
, avg(if(cont.message_length_avg>= no_cont.message_length_avg, 1,0)) as message_length_avg_better
, avg(if(cont.refactor_mle>= no_cont.refactor_mle, 1,0)) as refactor_mle_avg_better
...
not if(no_cont.status='Continue'
          ...
          , false)
and
cont_r.Is_Company = no_cont_r.Is_Company
group by f
having count(distinct cont.author_email) >= 100
order by f
;
...
# into continue_twins_by_dev_group.csv
SELECT
case
...
# into continue_twins_by_popularity_group.csv
SELECT
case
