...
def run_basic_models(concept):
    ...
    df = pd.read_csv(join(DATA_PATH
                          , 'two_year_developer_status.csv'))
    df = df[[i for i in df.columns if 'rel_' in i] + ['status']]
    #df = df[df.project_continued]

    keys = ['repo_name', 'author_email']
    metrics = list(set(df.columns) - NON_PREDICTIVE_FEATURES)

    """
    analyzed_year = 2012
    df = df[df.year.isin([analyzed_year, analyzed_year-1])]
    steps_df = build_two_steps_ds(df
                       , keys=keys
                       , metrics=metrics
                       , time_column='year'
                       , minimal_time=-1
    ...
    for model_name in classifiers.keys():
        ...
        regressor, performance = build_basic_model(df
                                                   ...
                                                   #, ablation_columns=[i for i in df.columns if 'rel_' not in i]
